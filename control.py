...
class Interface: 
	def __init__(self):
		...
		self.gmap = zeros((MAPH,MAPW),dtype=uint8)
		yaw = 0
		last_yaw = 0
		yaw_flag = False
		...
		while not rospy.is_shutdown():	
			...
			last_yaw = yaw		
			if(yaw_flag): yaw += pi						
			self.gmap = self.gmap*0.9 # when we get next scan frame, the map generated by last frame will be weaken
			...
			for r in self.scan_data.ranges:
				...
				#my = MAPH - int(py/PLENGTH + MAPH/2)
				#if(mx>=0 and mx<MAPW and my>=0 and my<MAPH): 
				#	self.gmap[my,mx]=128	# where is the robot now
				mx = int(gx/PLENGTH + MAPW/2)
				my = MAPH - int(gy/PLENGTH + MAPH/2)
				if(mx>=0 and mx<MAPW and my>=0 and my<MAPH): 
					self.gmap[my,mx]=255		
				scan_angle += self.scan_data.angle_increment
			cv2.imwrite("map.png",self.gmap)			
			...
			for p in obs:
				...
				if(min_d > dis):
					min_d = dis
			min_d = min_d*PLENGTH
			if(len(obs)): speed = (min_d-SECURE_DIS)*0.5 # speed will be slow when robot near obstacles
			...
			for s in alter_steer: # choose the best steer in alter set
				...
				for t in range(6): # predict the sport of robot in next 3 seconds by evaluate function
					...
					my = MAPH - int(future_y/PLENGTH + MAPH/2)
					eva += self.evaluate(my,mx,future_yaw)
				if(eva > max_eva):
					max_eva = eva
					steer = s
			if(speed==0): steer = 0.5 # if robot is not move ,rotate it to find a way
